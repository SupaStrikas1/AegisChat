...
const updateProfile = async (req, res) => {
  try {
    const { name, username, bio, interests, profilePic, fcmToken } = req.body;
    const updates = { name, username, bio, interests, profilePic, fcmToken };
...
const sendFriendRequest = async (req, res) => {
  const { toId } = req.body;
  if (toId === req.user._id.toString()) return res.status(400).json({ msg: 'Cannot send to self' });

  try {
    const toUser = await User.findById(toId);
    if (!toUser) return res.status(404).json({ msg: 'User not found' });

    if (req.user.friends.includes(toId)) return res.status(400).json({ msg: 'Already friends' });
    ...
    if (toUser.fcmToken) {
      admin.messaging().send({
        ...
        notification: { title: 'Friend Request', body: `${req.user.name} sent a friend request` },
      });
    }
...
const searchUsers = async (req, res) => {
  ...
  try {
    const users = await User.find({
      ...
      _id: { $ne: req.user._id },
    }).select('name username profilePic interests').limit(10);
    res.json(users);
  } catch (err) {
    res.status(500).json({ msg: 'Server error' });
  }
...
const getRecommendations = async (req, res) => {
  try {
    ...
    const recommended = await User.aggregate([
      ...
      {
        $addFields: {
          commonInterests: { $size: { $setIntersection: ['$interests', user.interests] } },
        },
      },
...
const acceptFriendRequest = async (req, res) => {
  ...
  try {
    ...
    if (!request || request.to.toString() !== req.user._id.toString()) return res.status(400).json({ msg: 'Invalid' });
...
const getFriends = async (req, res) => {
  try {
    const user = await User.findById(req.user._id).populate('friends', 'name email profilePic online');
...
module.exports = { updateProfile, sendFriendRequest, acceptFriendRequest, getFriends, searchUsers, getRecommendations, getFriendRequests, rejectFriendRequest };